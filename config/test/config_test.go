...
func TestRead(t *testing.T) {
	...
	assert.Equal(t, config.GetValueString("base.redis.standalone.addr"), "redis-service:26379")
	assert.Equal(t, config.RedisCfg.Standalone.Addr, "redis-service:26379")
	config.ClearConfig()
}

func TestReadLocal78(t *testing.T) {
	os.Setenv("gole.profile", "local78")
	config.LoadConfig()
	if config.ExistConfigFile() && config.GetValueBoolDefault("base.redis.enable", false) {
		err := config.GetValueObject("base.redis", &config.RedisCfg)
	...
	assert.Equal(t, config.GetValueString("base.redis.standalone.addr"), "10.30.30.78:26379")
	assert.Equal(t, config.RedisCfg.Standalone.Addr, "10.30.30.78:26379")
	config.ClearConfig()
}
